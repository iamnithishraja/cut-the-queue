		...
		const intervalId = setInterval(
			async () => {
				try {
					...
					if (
						!updatedOrderItem ||
						updatedOrderItem.status !== "WAITING_FOR_PICKUP"
					) {
					...
					console.error(
						"Failed to check order status or send notification:",
						error
					);
				}
			},
...
async function finishOrder(req: CustomRequest, res: Response): Promise<void> {
	...
	try {
		const result = await prisma.$transaction(async (tx) => {
			...
			const allItemsSent = order.OrderItem.every(
		...
		const statusCode =
			errorMessage === "No valid order items found" ? 400 : 500;
...
export {
	chageToPickup,
	finishOrder,
	getAllOrdersByCanteenId,
	getOrderDetailsByDay,
	updateCounter,
	updateItem,
};
