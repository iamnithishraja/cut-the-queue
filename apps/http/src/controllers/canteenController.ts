...
const calculateAmountForOrder = async (req: Request, res: Response) => {
	try {
		...
		if (dishes.length != orderItemList.length) {
			res.status(404).json({ message: ORDER_MISMATCHED });
			return;
		}
...
const toggleCanteenAvailability = async (req: Request, res: Response) => {
	try {
		...
		res.status(200).json({
			success: true,
			message: newStatus ? CANTEEN_OPENED : CANTEEN_CLOSED,
			isOpen: newStatus,
		});
	} catch (error) {
	...
	try {
		...
		if (data.mimeType) {
			try {
				...
				res.status(400).json({ message: IMAGE_UPLOAD_FAILED });
				return;
			}
		}
...
const editMenuItem = async (req: CustomRequest, res: Response) => {
	try {
		...
		if (!canteenId) {
			res.status(403).json({ message: NO_CANTEEN_ASSOCIATED });
			return;
		}
		...
		if (!existingMenuItem) {
			res.status(404).json({ message: MENU_ITEM_NOT_FOUND });
			return;
		}
...
const getCanteenAvilabality = async (req: Request, res: Response) => {
	try {
		...
		if (!canteen) {
			res.status(404).json({ message: CANTEEN_NOT_FOUND });
			return;
		}
